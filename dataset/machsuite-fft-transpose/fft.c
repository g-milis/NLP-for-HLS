...
L1:    int reversed8[8] = {0,4,2,6,1,5,3,7};
    int j;
    TYPE phi, tmp, phi_x, phi_y;

L2:    twiddles:for(j=1; j < 8; j++){
...
L3:    int reversed[8] = {0,4,2,6,1,5,3,7};
L4:    TYPE DATA_x[THREADS*8];
L5:    TYPE DATA_y[THREADS*8];
...
L8:    TYPE smem[8*8*9];
    ...
    //Do it all at once...
L9:loop1: for(tid = 0; tid < THREADS; tid++){
            //GLOBAL_LOAD...
        ...
        int sx, offset;
        sx = 66;
L10:loop2 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = hi*8+lo;
        ...
        sx = 8;
L11:loop3 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = lo*66+hi;
        ...
        sx = 66;
L12:loop4 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = hi*8+lo;
...
L13:loop5 : for(tid = 0; tid < 64; tid++){
...
L14:loop6 : for(tid = 0; tid < 64; tid++){
        ...
        //Transpose..
        sx = 72;
L15:loop7 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = hi*8+lo;
            smem[0*sx+offset] = DATA_x[tid*8 + 0];
        ...
        sx = 8;
L16:loop8 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = hi*72+lo;
        ...
        sx = 72;
L17:loop9 : for(tid = 0; tid < 64; tid++){
            hi = tid>>3;
            lo = tid&7;
            offset = hi*8+lo;
...
L18:loop10 : for(tid = 0; tid < 64; tid++){
             data_y[0] = DATA_y[tid*8 + 0];
             data_y[1] = DATA_y[tid*8 + 1];
             data_y[2] = DATA_y[tid*8 + 2];
             data_y[3] = DATA_y[tid*8 + 3];
...
L19:loop11 : for(tid = 0; tid < 64; tid++){
             //Load post-trans
