...
{
	int num_transfers = 9216; //(NCLUSTERS*NFEATURES) / WIDTH_FACTOR;
	int remainder = (NCLUSTERS*NFEATURES) % WIDTH_FACTOR;
L1:	for (int i(0); i<9216; ++i){
...
{
	if (flag){
		int num_transfers = 9216;//(TILE_SIZE * NFEATURES) / WIDTH_FACTOR;
		int start_idx = tile_idx * ((TILE_SIZE * NFEATURES) / WIDTH_FACTOR);
L2:		for (int i(0); i<9216; ++i){
...
{
	if (flag){
		int num_transfers = 9216; //TILE_SIZE / WIDTH_FACTOR;
		int start_idx = tile_idx * (TILE_SIZE / WIDTH_FACTOR);
L6:		for (int i(0); i<9216; ++i){
...
L7:    int local_membership_0[TILE_SIZE];
L8:    float local_feature_0[TILE_SIZE * NFEATURES];

L9:    int local_membership_1[TILE_SIZE];
L10:    float local_feature_1[TILE_SIZE * NFEATURES];

L11:    int local_membership_2[TILE_SIZE];
L12:    float local_feature_2[TILE_SIZE * NFEATURES];

L13:    float local_clusters[NCLUSTERS * NFEATURES];

    load_local_cluster(local_clusters, clusters);

L14:    for (int i=0; i<NUM_TILES+2; ++i)
    {
