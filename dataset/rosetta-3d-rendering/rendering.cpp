...
L1:  RAST2: for ( bit16 k = 0; k < 100 /*max_index[0]*/; k++ )
  {
...
L2:  static bit8 z_buffer[MAX_X][MAX_Y];
  if (counter == 0)
  {
L3:    ZCULLING_INIT_ROW: for ( bit16 i = 0; i < MAX_X; i++)
    {
...
L5:  ZCULLING: for ( bit16 n = 0; n < 100 /*size*/; n++ ) 
  {
...
L6:    COLORING_FB_INIT_ROW: for ( bit16 i = 0; i < MAX_X; i++)
    {
...
L8:  COLORING_FB: for ( bit16 i = 0; i < 10000 /*size_pixels*/; i++)
  {
...
void output_FB(bit8 frame_buffer[MAX_X][MAX_Y], bit32 output[NUM_FB])
{
  #pragma HLS INLINE
  bit32 out_FB = 0;
L9:  OUTPUT_FB_ROW: for ( bit16 i = 0; i < MAX_X; i++)
  {
...
extern "C" 
{
  void rendering( bit32 input[3*NUM_3D_TRI], bit32 output[NUM_FB])
  {
    ...
    bit16 size_fragment;
L11:    CandidatePixel fragment[500];
  
    bit16 size_pixels;
L12:    Pixel pixels[500];
  
L13:    bit8 frame_buffer[MAX_X][MAX_Y];
    bit2 angle = 0;
  
L14:    bit8 max_min[5];
L15:    bit16 max_index[1];
    bit2 flag;
  
    // processing NUM_3D_TRI 3D triangles
L16:    TRIANGLES: for (bit16 i = 0; i < NUM_3D_TRI; i++)
    {
